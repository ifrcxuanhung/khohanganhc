...
add index `y` (`y`), add index `x` (`x`) , add index `order_x` (`order_x`) , add index `order_y` (`order_y`);");
		...
		$sql_xx = "select max(order_x) as x from bga_array";
		$data_xx = $this->db->query($sql_xx)->row_array();
		
		$sql_x = "select * from bga_array";
		$data_x = $this->db->query($sql_x)->result_array();
		
		$count = $data_xx['x'];
		
		$sql_y = "select max(order_y) as y from bga_array";
		$data_y = $this->db->query($sql_y)->row_array();
		$nr = $data_y['y'];
		$kr = $count;
		//
		//
		...
		for($i=1; $i<=$nr; $i++){
			//$a = array(); // array of cells for column $c
			for($j=1; $j<=$kr; $j++){
		...
		//	for($i=1; $i<=$nr; $i++){
				//$a = array(); // array of cells for column $c
				//for($j=1; $j<=$kr; $j++){	
				$a[$value['order_y']][$value['order_x']] = $value['name'];	
				$a[0][$value['order_x']] = $value['x'];
				$a[$value['order_y']][0] = $value['y'];
		...
		if(!file_exists("import/bgamap.txt")){ 
			die('File does not exist');
		}

		clearstatcache();
		$myfile = fopen("import/bgamap.txt", "w") or die("Unable to open file!");
	...
	public function create_bumpmap(){
		...
		$this->db->query("drop table if exists bump_array");
		$this->db->query("create table bump_array select bumpname as name,concat('x',round((100* x)) ) `x`, round(y,2) y from bump_list") ;
		$this->db->query("alter table `bump_array` add column `order_x`  int(10) null after `name`,add column `order_y`  int(10) null after `order_x`,
		add index `y` (`y`), add index `x` (`x`) , add index `order_x` (`order_x`) , add index `order_y` (`order_y`)");
		
		$this->db->query("drop table if exists tmp_test_array");
		$this->db->query("create table tmp_test_array select x from bump_array group by x order by x asc");
		$this->db->query("alter table `tmp_test_array` add column `order`  int not null auto_increment after `x`,add primary key (`order`),add index `x` (`x`)");
		$this->db->query("update bump_array as a, tmp_test_array as b set a.order_x=b.`order` where a.x=b.x");
		
		$this->db->query("drop table if exists tmp_test_array");
		$this->db->query("create table tmp_test_array select y from bump_array group by y order by y asc");
		$this->db->query("alter table `tmp_test_array` add column `order`  int not null auto_increment after `y`,add primary key (`order`),add index `y` (`y`)");
		$this->db->query("update bump_array as a, tmp_test_array as b set a.order_y=b.`order` where a.y=b.y");
		
		$sql_xx = "select max(order_x) as x from bump_array";
		$data_xx = $this->db->query($sql_xx)->row_array();
		
		$sql_x = "select * from bump_array";
		$data_x = $this->db->query($sql_x)->result_array();
		
		$count = $data_xx['x'];
		
		$sql_y = "select max(order_y) as y from bump_array";
		$data_y = $this->db->query($sql_y)->row_array();
		$nr = $data_y['y'];
		$kr = $count;
		//
		...
		if(!file_exists("import/bumpmap.txt")){ 
			die('File does not exist');
		}

		clearstatcache();
		$myfile = fopen("import/bumpmap.txt", "w") or die("Unable to open file!");
		fwrite($myfile, $result);
		fclose($myfile);
		//

		//file_put_contents(base_url()."file.txt", $result);

		//$sql = "select `name`,order_x, order_y from test_array";
		//$data = $this->db->query($sql)->result_array();
		
	}
	
	public function create_from_test_array(){
		$sql_x = "select * from test_array";
		$data_x = $this->db->query($sql_x)->result_array();
		$count = count($data_x);
		
		$sql_y = "select max(order_y) as y from test_array";
		$data_y = $this->db->query($sql_y)->row_array();
		$nr = $data_y['y'];
		$kr = $count;
		//
    ...
    public function jq_loadtable(){
		...
		$table_export = $_REQUEST['table_export'];
		$table_import = $_REQUEST['table_import'];
		$page = $_REQUEST['page']; 
		...
		if($table_export == 'export'){
			$result = $this->jq_loadtable_model->getTableExport($page,$limit,$sord,$sidx,$filter,$filter_get,'web_summary');
		}
		elseif($table_import == 'import'){
			$result = $this->jq_loadtable_model->getTableImport($page,$limit,$sord,$sidx,$filter,$filter_get,'web_summary');
		}
		else{
	...
	public function get_all_code_product(){
		$sql = "SELECT code_product FROM khohang";
	...
	public function edit_del_add_jq_loadtable(){
		...
		if($_POST['oper'] == 'add'){
			if($_REQUEST['jq_table'] == 'khohang'){
				$all_code_product = $this->get_all_code_product();
				$code_product = $_POST['code_product'];
				if(in_array($code_product,$all_code_product)){ 
						echo "false";
				}else{
					...
					$data['userupdate'] = $_SESSION['simulation']['username'];
					...
					if(($table =='data_report') && (strtolower($item['field'])=='update') && (date('Y-m-d',strtotime("now")) == date('Y-m-d',strtotime($arr_data[$x][strtolower($item['field'])])))){
						...
						$records["data"][$key][] = '<div'.$align.'><a class="btn btn-icon-only green" href="http://'.$arr_data[$x][strtolower($item['field'])].'" target="_blank">
							...
							$records["data"][$key][] = '<div'.$align.'><a class="btn btn-icon-only green" href="'.$arr_data[$x][strtolower($item['field'])].'" target="_blank">
					 		...
					 		$sql_st = "SELECT `value` FROM setting WHERE `key_setting` = 'color_red'";
...
//        $this->db->where('user_log.user_id =', $this->session->userdata('user_id'));
	...
	public function index_last(){
		$block = new Block();
		$status_market = $this->db3->query("SELECT value FROM setting  WHERE `key_setting` = 'market_making_seconds'")->row_array();
...
group by df.type ORDER BY vcr.`name` asc")->result_array();
		...
		//option best limit
			$sql = "SELECT * ";
						$sql.= "FROM dashboard ";
						$sql.= "WHERE type = 'Call' and (nr BETWEEN -2 AND 2) and expiry = '{$_SESSION['session_expiry_date']}' and  dsymbol = '".$_SESSION['array_other_product']['dsymbol']."' order by nr LIMIT 0,5  ";
			...
			$sum_b_option = $this->db3->query("select sum(quantity) as b from vdm_order_options_daily where dsymbol = '".$_SESSION['array_other_product']['dsymbol']."' and `b/s` = 'B' and expiry = '{$_SESSION['session_expiry_date']}' and type ='C'")->row_array();
			$sum_s_option = $this->db3->query("select sum(quantity) as s from vdm_order_options_daily where dsymbol = '".$_SESSION['array_other_product']['dsymbol']."' and `b/s` = 'S' and expiry = '{$_SESSION['session_expiry_date']}' and type ='C'")->row_array();
			
			$avg_b_option = $this->db3->query("select avg(price) as b from vdm_order_options_daily where dsymbol = '".$_SESSION['array_other_product']['dsymbol']."' and `b/s` = 'B' and expiry = '{$_SESSION['session_expiry_date']}' and type ='C'")->row_array();
			$avg_s_option = $this->db3->query("select avg(price) as s from vdm_order_options_daily where dsymbol = '".$_SESSION['array_other_product']['dsymbol']."' and `b/s` = 'S' and expiry = '{$_SESSION['session_expiry_date']}' and type ='C'")->row_array();
    ...
    function view_modal() {
        ...
        if($type == 'input'){
            $html_code = '<input type="text" id="'.strtolower($field).'" name="'.strtolower($field).'" value="'.$value.'" class="input form-control key_down"/>';	   
        }else if ($type == 'choose'){
	...
	public function update_vndmi_setting(){
		...
		if($_REQUEST['open_close'] == 'false'){
			$data = array('value' => -1);
			$this->db3->where('group', 'setting')->where('key_setting','market_making_seconds')->update('setting', $data);	
		}else{
	...
	public function market_making_futures_close(){
		...
		$sql = "UPDATE setting SET value = 0 WHERE `key_setting` ='market_making_futures';";
        $this->db3->query($sql);
        return true;
    }
	public function market_making_futures_open(){

		$sql = "UPDATE setting SET value = 1 WHERE `key_setting` ='market_making_futures';";
        $this->db3->query($sql);
        return true;
    }
	
	public function update_list_table(){
		$getAll = $_REQUEST['getAll'];
		$date = date('Y-m-d H:i:s');
		$user = $_SESSION['simulation']['username'];
		foreach($getAll as $val){
			$table = strtolower($val[0]);
			
			$bumpname = strtolower($val[1]);
			$text1 = $val[2];
			$text2 = $val[3];
			$active = $val[4];
			if($active == 1){
				$sql = "UPDATE $table SET $bumpname = '$text2' WHERE $bumpname = '$text1'" ;
				//echo $sql;exit;
				$this->db->query($sql);
				$sql2 = "UPDATE swap SET status = 'DONE', dateupdate = '$date', userupdate = '$user' WHERE table_name = '$val[0]'" ;
				//echo $sql2;exit;
				$this->db->query($sql2);
				// update date
				$this->db->where('table_name', $table);
				$this->db->update('web_summary', array('dateupdate'=>date('Y-m-d H:i:s')));
			}
		}
		echo true;
		
	}
	public function import_file_txt(){
		$getAll = $_REQUEST['getAll'];
		//$user = $_SESSION['simulation']['username'];
    ...
    public function import($table){
		...
		$base_url = base_url();
		//$sql = "LOAD DATA LOCAL INFILE 'C:/xampp/htdocs/apm/import/{$table}.txt' INTO TABLE `{$table}` FIELDS TERMINATED BY  '\t'  LINES TERMINATED BY  '\r\n' IGNORE 1 LINES;";	
		//$sql = "LOAD DATA LOCAL INFILE '".$base_url."import/{$table}.txt' INTO TABLE `{$table}` FIELDS TERMINATED BY  '\t'  LINES TERMINATED BY  '\r\n' IGNORE 1 LINES;";
		if($_SERVER['HTTP_HOST'] == 'linux.itvn.fr'){
			$sql = "load data infile '"."{$table}.txt' INTO TABLE `{$table}` character set utf8 FIELDS TERMINATED BY  '\t'  LINES TERMINATED BY  '\r\n' IGNORE 1 LINES;";
			$this->db->query($sql);
		}else{
			$sql = "LOAD DATA LOCAL INFILE '".$base_url."import/{$table}.txt' INTO TABLE `{$table}` FIELDS TERMINATED BY  '\t'  LINES TERMINATED BY  '\r\n' IGNORE 1 LINES;";	
			$this->db->query($sql);
	...
	public function export_copy($table){
		...
		//file_put_contents($table.'.csv', 'hung');
		/*
		 2
		down vote
		...
		*/
		...
		$this->export_to_csv("{$table_sys}", $query, $aColumnsHeader, null,",", true);
	...
	public function export($table){
		...
		$headers = $this->db->query("SHOW COLUMNS FROM {$table}")->result_array();	
		//}
		...
		foreach ($headers as $item) {
			//$aColumnsHeader[]=$item['label'];
			if($item['Field'] != 'id' && $item['Field'] != 'userupdate' && $item['Field'] != 'dateupdate'){
				$aColumns[] = '`'.$item['Field'].'`';
				$aColumnsHeader[] = strtoupper($item['Field']);
			}
		}
        unset($headers); //$category == 'all' ? "efrc_".$table : "(SELECT * FROM efrc_".$table." where category = '".$category."') as sTable " ;
		...
		*/
		...
		$this->export_to_csv("{$table}", $query, $aColumnsHeader, null,",", false);
	...
	{   
		...
		$folder_setting = $this->db->query("select value from setting where key_setting = 'export_directory'")->row_array();
		
		if (!is_dir(dirname($folder_setting['value']))) {
			if (true !== @mkdir(dirname($folder_setting['value']), 0777, TRUE)) {
				if (is_dir(dirname($folder_setting['value']))) {
		...
		if(file_exists($folder_setting['value'].$rpName.'.csv')) {
			unlink($folder_setting['value'].$rpName.'.csv');
		}
		//mkdir($folder_setting['value'], 0777, true);
		
		//mkdir($folder_setting['value'], 0777, true);
		//file_put_contents($folder_setting['value'].$rpName.'_'.date('Ymd').'.csv', $content);
		file_put_contents($folder_setting['value'].$rpName.'.csv', $content);
		//$f = @fopen($folder_setting['value'].$rpName.'_'.date('Ymd').'.csv', 'w'); //this one seems to work and connect
		//fwrite($f, $content); // this one seems to work either and returning the number of byes..
		//fclose($f);
	...
	public function checkuser_exists(){
		$masp = $_REQUEST['masp'];
		$result = $this->db->where('code_product',$masp)->count_all_results('khohang');
		echo $result;
	}
