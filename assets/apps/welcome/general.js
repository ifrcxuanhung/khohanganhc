$(document).ready(function() {
	...
	$('.show_bummap').click(function(event){
			bootbox.confirm($confirm_run_bump_map, function(result) {
			...
			if(result == false){
			   ...
			   $.ajax({
					url: $base_url + 'ajax/create_bumpmap',
					type: 'POST',
					async: false,
					 beforeSend: function(){
						 $(".loader").show();
	 ...
	 $('.show_bgamap').click(function(event){
			bootbox.confirm($confirm_run_bga_map, function(result) {
		  	//Example.show("Confirm result: "+result);
			if(result == false){
					return true;
		   }
				...
				$.ajax({
					url: $base_url + 'ajax/create_bgamap',
					type: 'POST',
					async: false,
					 beforeSend: function(){
						 $(".loader").show();
	...
	$('.click_show_box').click(function(event){
			bootbox.confirm($text_login, function(result) {
		  	//Example.show("Confirm result: "+result);
			if(result == false){
				return true;
		   }
		...
		// return false;
			...
			if(result == false){
				   ...
				   $.ajax({
					...
					}else if(open_close == true && status != 'open'){
						$.ajax({
							type: "POST",
    ...
    //        });   
		...
		$('.key_downwww').keydown(function(e){
