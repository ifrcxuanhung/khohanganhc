...
<?php }?>
.page-header .navbar .navbar-nav > li > a{color:#<?php echo $text_menu_color['value'];?> !important;}
        ...
        <!--div class="container-fluid"-->
            <div class="clearfix navbar-fixed-top">
                ...
                <a id="index" class="page-logo" href="<?php echo $simulation_url; ?>" style="text-decoration: none;">
                    <strong style="color: #e4ad36"><font style="color:#009dc7"><?php echo $logo_txt_left['value']; ?></font> <?php echo $logo_txt_right['value'] ?></strong>
                </a>
            	<!-- END LOGO -->
                <!-- BEGIN SEARCH -->
               ...
               <!-- <form class="search hidden-sm" action="#" method="GET">
                ...
                <div class="topbar-actions">
                        ...
                        <div class="btn-group">
                            ...
                            <?php } else { ?>
                              <a href="<?php echo $link ?>" class="btn tooltips default <?php echo ($uj['class']=='bg_green')? 'bg_green':'bg_red'?>" data-container="body" data-placement="bottom" data-original-title="<?php echo $uj['name'] ?>">
                              <?php echo $uj['text'] ?>
                              </a>
                             <?php } ?>
                    ...
                    <div class="btn-group md-time">
                      <!--  <a class="btn red timer">
                       ...
                       <!-- <?php if((int)$status_market["value"] != -1) { ?>
                        	<a class="btn bg_green_head status_market"><?php translate('header_open'); ?></a>
                        <?php } else { ?>
                        <a class="btn bg_red_head status_market"><?php translate('header_close'); ?></a>
                        <?php } ?>-->
            ...
            <ul class="nav navbar-nav pull-right">
                ...
                if(isset($_SESSION['simulation']['username'])) {
                    ...
                    <ul class="dropdown-menu" style=" background:#fff !important; margin-top:10px;">
                        <li>
         ...
         <?php }?>
            ...
            <?php if($this->router->fetch_class() != 'start' && $this->router->fetch_class() != 'simulation'){?>
                <div class="nav-collapse collapse navbar-collapse navbar-responsive-collapse">
					...
					//echo "<pre>";print_r($active);
					foreach($simul_menu as $simul){
						//echo "<pre>";print_r($simul['url']);exit;
						$url_summary = end(explode('/',$simul['url']));
						?>
                        <li class="dropdown <?php if($active == $url_summary){?> dropdown-fw dropdown-fw-disabled active open selected<?php }?>">
                            <a <?php if($simul['url'] == 'bummap' || $simul['url'] == 'bgamap'){?>href="javascript:void(0);" <?php }else{?>href="<?php echo $simulation_url; ?><?php echo $simul['url'];?>"<?php }?> class="text-uppercase <?php if($simul['url'] == 'bummap'){ echo "show_bummap"; }?> <?php if($simul['url'] == 'bgamap'){ echo "show_bgamap"; }?>">
