...
GROUP BY df.dsymbol ORDER BY vcr.order ASC ";
    ...
    public function table_trading($code = '')
    {
		...
		//echo "<pre>";print_r($row_future['bid']);exit;
...
where vofd.dsymbol = '".$_SESSION['array_other_product']['dsymbol']."' and vofd.`b/s` = 'B' and vofd.expiry = '{$_SESSION['session_expiry_date']}' and vofd.price <= $row_future[bid]")->row_array();
		...
		$this->data->maxspd = $this->db3->query("select maxspd,tick from vdm_contracts_setting_exc where product = 'FUTURES' AND dsymbol = '".$_SESSION['array_other_product']['dsymbol']."' and expiry = '{$_SESSION['session_expiry_date']}'")->row_array();
...
where lang_code='en' and clean_scat in ('books','video','media') order by date_update desc;";
        ...
        $sql_quiz=" select * from simul_settings where stype='quiz' order by `order` asc; ";
    ...
    {
        $sql_analytics=" select * from simul_settings where stype='analytics' order by `order` asc; ";
    ...
    {
        $sql_faq="select * from simul_settings where stype='faq' and active=1 order by `order` asc;";
    ...
    {   
		$sql = "SELECT * ";
                    $sql.= "FROM dashboard ";
                    $sql.= "WHERE type = 'Call' and (nr BETWEEN -2 AND 2) and expiry = '{$_SESSION['session_expiry_date']}' and  dsymbol = '{$dsymbol}' order by nr LIMIT 0,5  ";
